...
%
(Problems might range from a teenager playing online games in the middle of
the night, to a child coming to parents in tears because something they saw on
the internet scared him/her.)

\item Kindsicher's loggin feature means that if black hat malware tried to
attack the Kindsicher server, the attack would be logged, so technically
competent humans could address the problem.
%
Even a non-technically inclined parent could still see which computer in
the home was involved in the attack (either as a source or as a victim), and
take that computer to a repair shop for delousing and a anti-malware update.
